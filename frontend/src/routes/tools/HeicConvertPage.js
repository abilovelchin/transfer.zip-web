...
export default function HeicConvertPage({}) {
  ...
  const handleFiles = async (_files) => {
    ...
    const files = Array.from(_files).filter((file) =>
    ...
    if (files.length == 1) {
      ...
      for (let index = 0; index < files.length; index++) {
        ...
        let zipWriter = zipStream
          .writable(file.name.replace(/\.heic$/i, ".jpg"))
          .getWriter();
  ...
  return (
    <div>
      <GenericToolPage
        ...
        display={
          <span>
            <span className="text-primary">Convert HEIC</span> to JPG online
          </span>
        }
        subtitle={
          "Easily convert your HEIC files to JPG format with our online tool. Your photos remain completely private - even from us."
        }
      >
        <div className="mx-auto max-w-sm">
          <FileUpload
            ...
            showProgress={max != 0}
            accept={".heic"}
            progressElement={<Progress now={now} max={max} />}
          />
        </div>
      </GenericToolPage>
          ...
          <>
            <div className="mt-16">
              ...
              <p className="text-lg mb-2">
                Simply choose the HEIC files from your computer, and they will
                be converted instantly.
              </p>
              <p className="text-lg mb-2">
                You can then choose to share individual photos for free if
                needed.
              </p>
              <p className="text-lg mb-2">

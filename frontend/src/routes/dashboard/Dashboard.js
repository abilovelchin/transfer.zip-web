    ...
    <div className="bg-white">
      <div className="z-20 bg-white fixed w-full md:hidden">
        <div className="h-16 border-b flex items-center justify-between px-4">
          <div>
            <a href="#" className="">
              <span className="sr-only">transfer.javascript.az</span>
              ...
              <div className="-my-6 divide-y divide-gray-500/10">
                <div className="space-y-2 py-6">
                  {MAIN_MENU.map((item) => (
                    ...
                    >
                      <BIcon className={"me-2"} name={item.icon} />
                      {item.text}
                    </Link>
                  ))}
                  ...
                  <Disclosure as="div" className="-mx-3">
                    <DisclosureButton className="group flex w-full items-center rounded-lg py-2 pl-3 pr-3.5 text-base/7 font-semibold text-gray-800 hover:bg-gray-50">
                      <BIcon
                        center
                        aria-hidden="true"
                        name={"chevron-up me-2"}
                        className="flex-none group-data-[open]:rotate-180 transition-transform"
                      />
                      Tools
                    </DisclosureButton>
                  ...
                  <Link
                    ...
                    className="-mx-3 block rounded-lg px-3 py-2 text-base/7 font-semibold text-gray-800 hover:bg-gray-50"
                  >
                    <BIcon className={"me-2"} name={"gear"} />
                    Settings
                  </Link>
                </div>
                {/* <div className="py-6">
...
const SmallLink = ({ icon, text, to }) => {
  return (
    <Link
      to={to}
      className={`hover:bg-body-secondary grow text-start text-sm px-4 py-1 flex items-center rounded-lg hover:text-primary text-secondary font-semibold`}
    >
      <BIcon className={`text-lg me-2`} name={`${icon}`} />
      {text}
    </Link>
  );
};
...
export default function Dashboard({}) {
  ...
  const selectedTransfer = useMemo(
    () =>
      selectedTransferId
        ? transfers.find((x) => x.id === selectedTransferId)
        : null,
    [selectedTransferId, transfers]
  );
  const displayedTransferId = useMemo(
    () => showSidebar && selectedTransferId,
    [showSidebar, selectedTransferId]
  );
  ...
  const Button = ({ icon, text, to, onClick, className }) => {
    const isActive =
      to != "/app" && to != "/app/transfers"
        ? currentPage.startsWith(to)
        : currentPage == to || currentPage == `${to}/`;
    const activeClassParent = isActive
      ? "text-primary bg-body-secondary font-semibold "
      : "text-secondary font-semibold ";

    return (
      <button
        onClick={onClick || (() => navigate(to))}
        className={`hover:bg-body-secondary grow text-start text-sm px-4 py-2 flex items-center rounded-lg hover:text-primary ${activeClassParent} ${className}`}
      >
        <BIcon
          className={`text-lg me-2`}
          name={`${icon}${isActive ? "-fill" : ""}`}
        />
        {text}
      </button>
    );
    ...
    >
      ...
      <Wrapper>
        <div className="hidden lg:w-64 py-6 px-6 pt-12 border-r bg-white md:flex flex-col">
          <Link className="mb-4" to="/">
            <div className="flex flex-row items-center">
              <img alt="Company Logo" src={logo} className="h-10 w-auto me-1" />
          ...
          <div className="flex flex-col gap-y-1">
            <Link
              ...
              className="mb-1 text-center bg-primary hover:bg-primary-light text-white text-sm font-medium py-2 rounded-md"
            >
              New Transfer
              <BIcon className={"ms-1.5 text-xs"} name={"send-fill"} />
            </Link>
            <span className="text-sm font-medium text-gray-500 my-1">
              Dashboard
            </span>
            {MAIN_MENU.map((value, index) => (
              <Button key={value.to} {...value} />
            ))}
            <span className="text-sm font-medium text-gray-500 my-1">
              Tools
            </span>
            {SECONDARY_MENU.map((value, index) => (
              <SmallLink key={value.to} {...value} />
            ))}
          </div>
        ...
        <Transition show={showSidebar}>
          <div className="z-20 --overflow-hidden duration-0 data-[closed]:w-0 data-[leave]:overflow-hidden data-[enter]:overflow-hidden fixed top-0 w-[100vw] h-full md:relative md:h-auto md:duration-300 md:w-96 xl:w-[512px]">

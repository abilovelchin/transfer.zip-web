...
export default function QuickShareProgress({}) {
  ...
  const { displayNotification, setShowSignUpModal } =
    useContext(ApplicationContext);
  const { hasBeenSentLink, files, k, remoteSessionId, transferDirection } =
    useContext(QuickShareContext);

  const navigate = useNavigate();

  const [transferState, _setTransferState] = useState(
    hasBeenSentLink ? TRANSFER_STATE_IDLE : TRANSFER_STATE_WAIT_FOR_USER
  );
  ...
  const hasConnected = useMemo(
    () =>
      transferState == TRANSFER_STATE_TRANSFERRING ||
  ...
  const bytesTransferred = useMemo(() => {
    if (filesProgress) {
      ...
      return filesProgress.reduce(
        (total, fileProgress) => total + fileProgress.bytesTransferred,
        0
      );
    }
    return 0;
  }, [filesProgress]);
  const percentTransferred = useMemo(
    () => Math.floor((totalBytes / bytesTransferred) * 100),
  ...
  const startProgress = () => {
    ...
    const recvDirection = (fileTransfer, fileList) => {
      ...
      console.log(
        "[QuickShareProgress] [recvDirection] File list query has been received",
        fileList,
        "doZip:",
        doZip
      );
      const fileStream = streamSaver.createWriteStream(
        doZip ? "transfer.javascript.az" : fileList[0].name,
        {
      ...
      fileTransfer.onprogress = ({ now, max, done }, fileInfo) => {
        if (filesDone >= fileList.length)
          return console.warn(
            "[QuickShareProgress] fileTransfer.onprogress called after files are done"
          );
      ...
      if (doZip) {
        ...
        let currentZipWriter = zipStream
          .writable(fileList[0].relativePath)
          .getWriter();
        ...
        fileTransfer.onfilefinished = (fileInfo) => {
          ...
          if (filesDone >= fileList.length) {
            zipStream
              .close()
              .then(() => setTransferState(TRANSFER_STATE_FINISHED));
            return;
          }

          currentZipWriter = zipStream
            .writable(fileList[filesDone].relativePath)
            .getWriter();

          if (filesDone >= fileList.length) {
            console.error(
              "THIS SHOULD NOT HAPPEN: Requesting more files even when all files are downloaded!?"
            );
          } else {
        ...
        const transformer = new TransformStream({
          ...
          transform(data, controller) {
            ...
            while (dataOffset < data.length) {
              ...
              let bytesToWrite = Math.min(
                remainingSpaceInChunk,
                data.length - dataOffset
              );

              this.chunk.set(
                data.slice(dataOffset, dataOffset + bytesToWrite),
                this.offset
              );
    ...
    const onerror = (err) => {
      ...
      if (err instanceof WebRtc.PeerConnectionError) {
        ...
        setErrorMessage(
          err.message ||
            "Sorry an unknown error has occured! Check back later and we should hopefully have fixed it."
        );
      }
    };
    ...
    if (hasBeenSentLink) {
      ...
      call(remoteSessionId, k, totalBytes > 10_000_000)
        .then((fileTransfer) => {
          if (transferDirection == "R")
            fileTransferGetFileList(fileTransfer).then((fileList) =>
              recvDirection(fileTransfer, fileList)
            );
          else
            fileTransferServeFiles(fileTransfer, files).then(() =>
              sendDirection(fileTransfer)
            );
        })
        .catch(onerror);
    } else {
      ...
      const oncandidate = (candidate) => {
        ...
        waitTimer = setTimeout(() => {
          if (
            transferState == TRANSFER_STATE_WAIT_FOR_USER ||
            transferState == TRANSFER_STATE_CONNECTING
          ) {
            // setTransferState(TRANSFER_STATE_IDLE)
          }
        }, 15000);
      ...
      listen(
        (linkWithoutDirection) =>
          setQuickShareLink(linkWithoutDirection + directionCharForRecipient),
  ...
  const spinner = (
    <Spinner className={"inline-block"} sizeClassName={"h-4 w-4"} />
  );
  ...
  const handleCopy = async (e) => {
    if (await tryCopyToClipboard(quickShareLink)) {
      displayNotification(
        "Copied Link",
        "The Quick Share link was successfully copied to the clipboard!"
      );
    }
  };
        ...
        <div className="relative">
          <QRCode
            ...
            value={
              quickShareLink
                ? quickShareLink
                : "https://transfer.javascript.az/?542388234752394243924377293849asdasd"
            }
          />
          ...
          <div>
            <div className="relative mt-2 flex items-center">
              ...
              <div className="absolute inset-y-0 right-0 flex py-1.5 pr-1.5">
                <button
                  onClick={handleCopy}
                  className="inline-flex items-center rounded border border-gray-200 px-1 pe-1.5 font-sans text-xs text-primary font-medium bg-white hover:bg-gray-50"
                >
                  <BIcon name={"copy"} className={"mr-1 ms-1"} />
                  Copy
                </button>
              </div>
            </div>
      ...
      <div>
        ...
        {!errorMessage ? (
          <ol className="list-decimal list-inside mb-4 md:mb-2">
            ...
            <li
              className={
                transferState == TRANSFER_STATE_IDLE ? "" : "text-gray-400"
              }
            >
              {hasBeenSentLink && !isSelfHosted()
                ? "Connecting to server..."
                : "Scan the QR code or send the link to the recipient."}{" "}
              {transferState == TRANSFER_STATE_IDLE && spinner}
            </li>
            <li
              className={
                transferState == TRANSFER_STATE_CONNECTING
            ...
            >
              ...
              {transferState == TRANSFER_STATE_CONNECTING && spinner}
            </li>
            <li
              className={
                transferState == TRANSFER_STATE_TRANSFERRING
            ...
            >
              Stand by while the files are being transfered.{" "}
              {transferState == TRANSFER_STATE_TRANSFERRING && spinner}
            </li>
            <li
              className={
                transferState == TRANSFER_STATE_FINISHED ? "" : "text-gray-400"
              }
            >
              Done!
            </li>
          ...
          <Link
            ...
            onClick={(e) => {
              if (!user) {
                e.preventDefault();
                if (window.sa_loaded)
                  window.sa_event("quick-share_upsell_clicked");
          ...
          >
            <div className="flex items-center h-6">
              <BIcon
                center
                className={
                  "text-purple-500 text-sm animate-pulse group-hover:animate-none mt-1"
                }
                name={"lightning-fill"}
              />{" "}
            </div>
            <div>
              <p className="sm:text-lg font-bold text-purple-500">
                {hasBeenSentLink
                  ...
                  : "This link will expire when tab is closed."}
              </p>
              {!hasBeenSentLink && (
                <span className="text-purple-500 font-medium">
                  ...
                  <span className="transition-all group-hover:ms-1">
                    &rarr;
                  </span>
                </span>
              )}
